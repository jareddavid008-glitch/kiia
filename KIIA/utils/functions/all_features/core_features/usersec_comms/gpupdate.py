...
from ....allowed import SAFE_GPUPDATE_COMMANDS

class GpupdateCommandWindow(CommandWindow):
    def __init__(self, controller, title, gpupdate_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.gpupdate_args = gpupdate_args
        ...
        self.btn.clicked.connect(self.run_gpupdate)
    ...
    def run_gpupdate(self):
        self.start_process("cmd", ["/c"] + self.gpupdate_args)
...
class GpupdateInputWindow(CommandWindow):
...
class GpupdateSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Gpupdate command:"))

        self.commands = SAFE_GPUPDATE_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = GpupdateCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = GpupdateInputWindow(self.controller, title, spec["cmd"])
