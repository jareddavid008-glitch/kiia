...
from ....allowed import SAFE_NET_USER_COMMANDS

class NetuserCommandWindow(CommandWindow):
    def __init__(self, controller, title, netuser_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.netuser_args = netuser_args
        ...
        self.btn.clicked.connect(self.run_netuser)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_netuser(self):
        self.start_process("cmd", ["/c"] + self.netuser_args)
...
class NetuserInputWindow(CommandWindow):
...
class NetuserSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Net User command:"))

        self.commands = SAFE_NET_USER_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = NetuserCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = NetuserInputWindow(self.controller, title, spec["cmd"])
