...
from ....allowed import SAFE_RUNAS_COMMANDS

class RunasCommandWindow(CommandWindow):
    def __init__(self, controller, title, runas_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.runas_args = runas_args
        ...
        self.btn.clicked.connect(self.run_runas)
    ...
    def run_runas(self):
        self.start_process("cmd", ["/c"] + self.runas_args)
...
class RunasInputWindow(CommandWindow):
...
class RunasSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Runas command:"))

        self.commands = SAFE_RUNAS_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = RunasCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = RunasInputWindow(self.controller, title, spec["cmd"])
