...
from ....allowed import SAFE_NET_LOCALGROUP_COMMANDS

class NetlocalgroupCommandWindow(CommandWindow):
    def __init__(self, controller, title, netlocalgroup_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.netlocalgroup_args = netlocalgroup_args
        ...
        self.btn.clicked.connect(self.run_netlocalgroup)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_netlocalgroup(self):
        self.start_process("cmd", ["/c"] + self.netlocalgroup_args)
...
class NetlocalgroupInputWindow(CommandWindow):
...
class NetlocalgroupSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Net Local command:"))

        self.commands = SAFE_NET_LOCALGROUP_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = NetlocalgroupCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = NetlocalgroupInputWindow(self.controller, title, spec["cmd"])
