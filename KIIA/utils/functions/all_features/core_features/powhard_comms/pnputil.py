...
from ....allowed import SAFE_PNPUTIL_COMMANDS

class PnputilCommandWindow(CommandWindow):
    def __init__(self, controller, title, pnputil_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.pnputil_args = pnputil_args
        ...
        self.btn.clicked.connect(self.run_pnputil)
    ...
    def run_pnputil(self):
        self.start_process("cmd", ["/c"] + self.pnputil_args)
...
class PnputilInputWindow(CommandWindow):
...
class PnputilSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an pnputil command:"))

        self.commands = SAFE_PNPUTIL_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = PnputilCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = PnputilInputWindow(self.controller, title, spec["cmd"])
