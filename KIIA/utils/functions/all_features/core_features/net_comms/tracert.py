...
from ....allowed import SAFE_TRACERT_COMMANDS

class TracertCommandWindow(CommandWindow):
    def __init__(self, controller, title, tracert_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.tracert_args = tracert_args
        ...
        self.btn.clicked.connect(self.run_tracert)
    ...
    def run_tracert(self):
        self.start_process("cmd", ["/c"] + self.tracert_args)
...
class TracertInputWindow(CommandWindow):
...
class TracertSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Tracert command:"))

        self.commands = SAFE_TRACERT_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = TracertCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = TracertInputWindow(self.controller, title, spec["cmd"])
