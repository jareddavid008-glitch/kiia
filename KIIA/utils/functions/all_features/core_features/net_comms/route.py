...
from ....allowed import SAFE_ROUTE_COMMANDS

class RouteCommandWindow(CommandWindow):
    def __init__(self, controller, title, route_args, parent=None):
        ...
        self.btn.clicked.connect(self.run_route)
    ...
    def run_route(self):
        self.start_process("cmd", ["/c"] + self.route_args)
...
class RouteInputWindow(CommandWindow):
...
class RouteSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Route command:"))

        self.commands = SAFE_ROUTE_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = RouteCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = RouteInputWindow(self.controller, title, spec["cmd"])
