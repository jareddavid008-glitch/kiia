...
from ....allowed import SAFE_PATHPING_COMMANDS

class PathpingCommandWindow(CommandWindow):
    def __init__(self, controller, title, pathping_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.pathping_args = pathping_args
        ...
        self.btn.clicked.connect(self.run_pathping)
    ...
    def run_pathping(self):
        self.start_process("cmd", ["/c"] + self.pathping_args)
...
class PathpingInputWindow(CommandWindow):
...
class PathpingSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an PathPing command:"))

        self.commands = SAFE_PATHPING_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = PathpingCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = PathpingInputWindow(self.controller, title, spec["cmd"])
