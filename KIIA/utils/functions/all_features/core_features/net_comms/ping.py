...
from .....tools.logger import LoggedLineEdit, speak
from ....allowed import SAFE_PING_COMMANDS

class PingCommandWindow(CommandWindow):
    def __init__(self, controller, title, ping_args, parent=None):
        ...
        self.btn.clicked.connect(self.run_ping)
    ...
    def run_ping(self):
        self.start_process("cmd", ["/c"] + self.ping_args)
...
class PingInputWindow(CommandWindow):
...
class PingSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an ping command:"))

        self.commands = SAFE_PING_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = PingCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = PingInputWindow(self.controller, title, spec["cmd"])
