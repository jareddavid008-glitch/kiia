...
from ....allowed import SAFE_NSLOOKUP_COMMANDS

class NslookupCommandWindow(CommandWindow):
    def __init__(self, controller, title, nslookup_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.nslookup_args = nslookup_args
        ...
        self.btn.clicked.connect(self.run_nslookup)
    ...
    def run_nslookup(self):
        self.start_process("cmd", ["/c"] + self.nslookup_args)
...
class NslookupInputWindow(CommandWindow):
...
class NslookupSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Nslookup command:"))

        self.commands = SAFE_NSLOOKUP_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = NslookupCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = NslookupInputWindow(self.controller, title, spec["cmd"])
