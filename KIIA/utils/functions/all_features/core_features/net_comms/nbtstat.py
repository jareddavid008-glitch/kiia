...
from ....allowed import SAFE_NBTSTAT_COMMANDS

class NbtstatCommandWindow(CommandWindow):
    def __init__(self, controller, title, nbtstat_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.nbtstat_args = nbtstat_args
        ...
        self.btn.clicked.connect(self.run_nbtstat)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_nbtstat(self):
        self.start_process("cmd", ["/c"] + self.nbtstat_args)
...
class NbtstatInputWindow(CommandWindow):
...
class NbtstatSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Nbtstat command:"))

        self.commands = SAFE_NBTSTAT_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = NbtstatCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = NbtstatInputWindow(self.controller, title, spec["cmd"])
