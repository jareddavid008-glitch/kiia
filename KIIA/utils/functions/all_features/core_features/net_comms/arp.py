...
from ....allowed import SAFE_ARP_COMMANDS

class ArpCommandWindow(CommandWindow):
    def __init__(self, controller, title, arp_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.arp_args = arp_args
        ...
        self.btn.clicked.connect(self.run_arp)
    ...
    def run_arp(self):
        self.start_process("cmd", ["/c"] + self.arp_args)
...
class ArpInputWindow(CommandWindow):
...
class ArpSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an arp command:"))

        self.commands = SAFE_ARP_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = ArpCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = ArpInputWindow(self.controller, title, spec["cmd"])
