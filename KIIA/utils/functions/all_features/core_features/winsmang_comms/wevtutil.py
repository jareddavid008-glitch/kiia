...
from ....allowed import SAFE_WEVTUTIL_COMMANDS

class WevtutilCommandWindow(CommandWindow):
    def __init__(self, controller, title, wevtutil_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.wevtutil_args = wevtutil_args
        ...
        self.btn.clicked.connect(self.run_wevtutil)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_wevtutil(self):
        self.start_process("cmd", ["/c"] + self.wevtutil_args)
...
class WevtutilInputWindow(CommandWindow):
...
class WevtutilSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an wevtutil command:"))

        self.commands = SAFE_WEVTUTIL_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = WevtutilCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = WevtutilInputWindow(self.controller, title, spec["cmd"])
