...
from ....allowed import SAFE_AUDITPOL_COMMANDS

class AuditpolCommandWindow(CommandWindow):
    def __init__(self, controller, title, auditpol_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.auditpol_args = auditpol_args
        ...
        self.btn.clicked.connect(self.run_auditpol)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_auditpol(self):
        self.start_process("cmd", ["/c"] + self.auditpol_args)
...
class AuditpolInputWindow(CommandWindow):
...
class AuditpolSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an auditpol command:"))

        self.commands = SAFE_AUDITPOL_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = AuditpolCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = AuditpolInputWindow(self.controller, title, spec["cmd"])
