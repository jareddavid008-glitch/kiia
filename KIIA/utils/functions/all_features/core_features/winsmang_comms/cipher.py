...
from ....allowed import SAFE_CIPHER_COMMANDS

class CipherCommandWindow(CommandWindow):
    def __init__(self, controller, title, cipher_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.cipher_args = cipher_args
        ...
        self.btn.clicked.connect(self.run_cipher)
    ...
    def run_cipher(self):
        self.start_process("cmd", ["/c"] + self.cipher_args)
...
class CipherInputWindow(CommandWindow):
...
class CipherSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an cipher command:"))

        self.commands = SAFE_CIPHER_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = CipherCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = CipherInputWindow(self.controller, title, spec["cmd"])
