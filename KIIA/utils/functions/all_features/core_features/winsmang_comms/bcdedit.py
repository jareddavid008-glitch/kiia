...
from ....allowed import SAFE_BCDEDIT_COMMANDS

class BcdeditCommandWindow(CommandWindow):
    def __init__(self, controller, title, bcdedit_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.bcdedit_args = bcdedit_args
        ...
        self.btn.clicked.connect(self.run_bcdedit)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_bcdedit(self):
        self.start_process("cmd", ["/c"] + self.bcdedit_args)
...
class BcdeditInputWindow(CommandWindow):
...
class BcdeditSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Bcdedit command:"))

        self.commands = SAFE_BCDEDIT_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = BcdeditCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = BcdeditInputWindow(self.controller, title, spec["cmd"])
