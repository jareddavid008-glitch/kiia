...
from ....allowed import SAFE_SCHTASKS_COMMANDS

class SchtasksCommandWindow(CommandWindow):
    def __init__(self, controller, title, schtasks_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.schtasks_args = schtasks_args
        ...
        self.btn.clicked.connect(self.run_schtasks)
    ...
    def run_schtasks(self):
        self.start_process("cmd", ["/c"] + self.schtasks_args)
...
class SchtasksInputWindow(CommandWindow):
...
class SchtasksSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an schtasks command:"))

        self.commands = SAFE_SCHTASKS_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = SchtasksCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = SchtasksInputWindow(self.controller, title, spec["cmd"])
