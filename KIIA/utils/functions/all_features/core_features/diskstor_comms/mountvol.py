...
from ....allowed import SAFE_MOUNTVOL_COMMANDS

class MountvolCommandWindow(CommandWindow):
    def __init__(self, controller, title, mountvol_args, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.mountvol_args = mountvol_args
        ...
        self.btn.clicked.connect(self.run_mountvol)

        layout = QVBoxLayout(self)
        layout.addWidget(self.label)
        layout.addWidget(self.btn)

    def run_mountvol(self):
        self.start_process("cmd", ["/c"] + self.mountvol_args)
...
class MountvolInputWindow(CommandWindow):
...
class MountvolSafeMenu(QWidget):
    def __init__(self, controller):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Select an Mountvol command:"))

        self.commands = SAFE_MOUNTVOL_COMMANDS
    ...
    def launch_command(self, title, spec):
        if not spec["input"]:
            win = MountvolCommandWindow(self.controller, title, spec["cmd"])
        else:
            win = MountvolInputWindow(self.controller, title, spec["cmd"])
